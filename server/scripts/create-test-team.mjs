...
// 中奥致远的机构ID
const ORG_ID = 'e0cee8a1-ab21-4131-8088-78897c686ff3';
...
async function createTestTeam() {
  ...
  for (let i = 1; i <= 30; i++) {
    ...
    const memberType = i <= 25 ? 'employee' : 'external_support';
    ...
    console.log(`${i}. ${employeeCode} ${employeeName} - ${department} ${position} (${memberType === 'employee' ? '正式员工' : '外援'})`);
  }
  ...
  console.log(`   - 外援成员: ${insertedMembers.filter(m => m.member_type === 'external_support').length} 人`);
