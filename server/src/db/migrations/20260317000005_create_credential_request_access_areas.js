export async function up(knex) {
    await knex.schema.createTable('credential_request_access_areas', (t) => {
        t.bigIncrements('id').primary();
        t.bigInteger('request_id').notNullable()
            .references('id').inTable('credential_requests').onDelete('CASCADE');
        ...
        t.unique(['request_id', 'access_code']);
        t.index(['request_id']);
...
export async function down(knex) {
    await knex.schema.dropTableIfExists('credential_request_access_areas');
}
