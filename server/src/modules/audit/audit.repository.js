...
export async function lockRaceScope(orgId, raceId, database) {
  const row = await database('races').where({ id: raceId, org_id: orgId }).forUpdate().first('id')
  if (row) return { id: Number(row.id) }
  const error = new Error('Target race not found or forbidden')
  error.status = 404
  error.code = 'AUDIT_RACE_NOT_FOUND'
  error.expose = true
  throw error
}

export async function hasActiveAuditJobs(orgId, raceId, database = knex) {
  const row = await database('jobs')
    .where({ org_id: orgId, race_id: raceId })
    .whereIn('status', ['queued', 'running'])
    .where('type', 'like', 'audit:%')
    .first('id')
  return Boolean(row)
}
...
export async function getPrepStats(orgId, raceId) {
  const rows = await knex('records')
    ...
    .select(
      ...
      knex.raw(
        "SUM(CASE WHEN lottery_status IN ('直通名额','直通','强制保签') THEN 1 ELSE 0 END)::int AS direct",
      ),
      knex.raw(
        "SUM(CASE WHEN lottery_status IN ('不予通过','模糊剔除','未成年剔除','精英资质存疑','强制剔除') THEN 1 ELSE 0 END)::int AS denied",
      ),
      knex.raw(
        "SUM(CASE WHEN lottery_status IS NULL OR lottery_status = '' OR lottery_status NOT IN ('参与抽签','直通名额','直通','强制保签','不予通过','模糊剔除','未成年剔除','精英资质存疑','强制剔除') THEN 1 ELSE 0 END)::int AS pending",
      ),
...
export async function resetAudit(orgId, raceId, database = knex) {
  // 步骤 1：保留已有排除性 lotteryStatus 的记录 → reject + pre_excluded
  const preservedResult = await database('records')
    .where({ org_id: orgId, race_id: raceId })
  ...
  // 清除旧审核记录
  await database('audit_results').where({ org_id: orgId, race_id: raceId }).delete()
  await database('audit_runs').where({ org_id: orgId, race_id: raceId }).delete()

  // 获取 pending 计数
  const { cnt: pending } = await database('records')
...
export async function createRun(orgId, raceId, stepNumber, stepName, database = knex) {
  const row = {
    org_id: orgId,
    race_id: raceId,
  ...
  const [inserted] = await database('audit_runs').insert(row).returning('*')
  return auditRunMapper.fromDbRow(inserted)
}

export async function completeRun(orgId, raceId, runId, affected, remaining, database = knex) {
  const [updated] = await database('audit_runs')
    .where({ id: runId, org_id: orgId, race_id: raceId })
    .update({
      status: 'completed',
      affected,
      remaining,
    ...
    .returning('*')
  const run = auditRunMapper.fromDbRow(updated)
  if (run) return run
  const error = new Error('Audit run not found inside the job scope')
  error.code = 'AUDIT_RUN_SCOPE_MISMATCH'
  throw error
}

export async function failRun(orgId, raceId, runId, error, database = knex) {
  const [updated] = await database('audit_runs')
    .where({ id: runId, org_id: orgId, race_id: raceId })
    .update({
      status: 'failed',
