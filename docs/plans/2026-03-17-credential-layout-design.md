...
| 端 | 职责范围 | 关键词 |
|---|---------|--------|
| **ArcSpro (Web)** | 需求定义 → 领取管理 → 扫描核验 | 多少区域、多少岗位、多少证件；领取登记；扫码验证 |
| **TOOL (Electron)** | 排版编辑 → 地图框选 → 批量生成 | 类似号码布排版；地图截图嵌入；PDF 批量导出 |

## 二、现有组件归属审计

### ArcSpro 端现有 (6 页面 + API) — ✅ 基本正确

| 文件 | 归属 | 状态 |
|------|------|------|
| `CredentialZonePage.jsx` | ✅ ArcSpro (需求定义) | 通行区域 CRUD |
| `CredentialRolePage.jsx` | ✅ ArcSpro (需求定义) | 证件类别/岗位 CRUD |
| `CredentialApplicationPage.jsx` | ✅ ArcSpro (需求定义) | 证件申请管理 |
| `CredentialReviewPage.jsx` | ✅ ArcSpro (需求定义) | 审核流程 |
| `CredentialStylePage.jsx` | ⚠️ 职责重叠 | 模板管理 — 与 TOOL 端排版器功能重叠 |
| `ScanHome/ScanResult.jsx` | ✅ ArcSpro (扫描) | 已有扫码入口 |
| `credential.js` (API) | ✅ ArcSpro | 后端 API 封装 |
...
| `CredentialScanView.tsx` | ❌ **放错端** | 扫码应留在 ArcSpro 端进行，TOOL 端不需要 |
...
扫码核验是 ArcSpro 端的职责。ArcSpro 端已有完整的 `ScanHome.jsx` → `ScanResult.jsx` 扫码链路。TOOL 端的 `CredentialScanView.tsx` 是多余的重复实现，应删除，并从路由/侧边栏中移除入口。

### 调整 2: ArcSpro 端 `CredentialStylePage.jsx` 降级为"查看器"

模板的**详细排版编辑**是 TOOL 端的职责。ArcSpro 端的 `CredentialStylePage` 应保留模板的基本 CRUD（名称、编码、尺寸、状态），但不需要自带排版画布预览。可以保留只读预览，让用户知道模板长什么样，但编辑操作引导到 TOOL 端。

### 调整 3: ArcSpro 端 `ScanResult.jsx` 增加证件类型支持
