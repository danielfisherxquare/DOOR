...
test('audit job polling receives a surface-aware app or admin jobs path', () => {
  const source = read('src/api/audit.js')

  assert.equal(source.includes("app: '/app/jobs'"), true)
  assert.equal(source.includes("admin: '/admin/jobs'"), true)
  assert.match(source, /jobsBasePath:\s*getJobsBasePath\(\)/)
})
...
test('app event routes expose audit endpoints for the H5 processing surface', () => {
  ...
  assert.match(
    source,
    /app\.use\(['"]\/api\/app\/audit['"],\s*appEventsPermission,\s*auditRoutes\)/
  )
  assert.match(
    source,
    /app\.use\(['"]\/api\/admin\/audit['"],\s*requirePermission\(\{ surface: ['"]admin['"] \}\),\s*auditRoutes\)/
  )
})
