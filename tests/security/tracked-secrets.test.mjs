...
test('secret scanner distinguishes placeholders from credential-shaped values', () => {
  ...
  assert.equal(isPlaceholderSecretValue('test_database_password'), true)
  assert.equal(isPlaceholderSecretValue('1'.repeat(64)), true)
  assert.equal(isPlaceholderSecretValue('sk-live-example-value-that-must-be-rejected'), false)
  assert.equal(
    isPlaceholderSecretValue('97f5fd449ebd7955839c088e8e215f1c8ca101ac7f9b16986d20196750913b66'),
    false,
  )
})
...
test('secret scanner rejects sibling env files and non-placeholder assignments', () => {
  ...
  assert.deepEqual(
    findSecretAssignmentRules(
      'PII_ENCRYPTION_KEY_V1=97f5fd449ebd7955839c088e8e215f1c8ca101ac7f9b16986d20196750913b66',
    ),
    ['non-placeholder-secret-assignment'],
  )
})
