...
export const lotteryApi = {
    // ── race_capacity ────────────────────────────────────────
  getRaceCapacity: (raceId) => request.get(`${getBasePath()}/configs/${raceId}`).then(unwrapData),
  ...
  deleteLotteryList: (id) => request.delete(`${getBasePath()}/lists/entry/${id}`).then(unwrapData),
    ...
    // ── lottery_rules ────────────────────────────────────────
  getLotteryRules: (raceId) => request.get(`${getBasePath()}/rules/${raceId}`).then(unwrapData),

  saveLotteryRule: (data) => request.post(`${getBasePath()}/rules`, data).then(unwrapData),

    // ── lottery_weights ──────────────────────────────────────
  getLotteryWeights: (raceId) => request.get(`${getBasePath()}/weights/${raceId}`).then(unwrapData),

  saveLotteryWeight: (data) => request.post(`${getBasePath()}/weights`, data).then(unwrapData),

  deleteLotteryWeight: (id) => request.delete(`${getBasePath()}/weights/${id}`).then(unwrapData),
  ...
  getLotteryResults: (raceId) => request.get(`${getBasePath()}/results/${raceId}`).then(unwrapData),

    hasSnapshot: (raceId) =>
    request
      .get(`${getBasePath()}/has-snapshot/${raceId}`)
      .then((response) => Boolean(unwrapData(response)?.hasSnapshot)),

  rollbackLottery: (raceId) => request.post(`${getBasePath()}/rollback/${raceId}`).then(unwrapData),
