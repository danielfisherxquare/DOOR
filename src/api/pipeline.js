...
export const pipelineApi = {
    // ── start_zones ──────────────────────────────────────────
  getStartZones: (raceId) => request.get(`${getBasePath()}/start-zones/${raceId}`).then(unwrapData),

  saveStartZone: (data) => request.post(`${getBasePath()}/start-zones`, data).then(unwrapData),

  deleteStartZone: (id) => request.delete(`${getBasePath()}/start-zones/${id}`).then(unwrapData),
    ...
    // ── Phase 6: Pipeline 执行 ────────────────────────────────
  executePipeline: (raceId) => request.post(`${getBasePath()}/execute/${raceId}`).then(unwrapData),

    getExecutionStatus: (executionId) =>
        request.get(`${getBasePath()}/execution/${executionId}`).then(unwrapData),

  getPreview: (raceId) => request.get(`${getBasePath()}/preview/${raceId}`).then(unwrapData),

  previewPipeline: (raceId) => pipelineApi.getPreview(raceId),
