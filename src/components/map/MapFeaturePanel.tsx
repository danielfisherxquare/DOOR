...
interface ExportWorkZoneCandidate {
  id?: string;
  updatedAt?: string | null;
  metadata?: {
    purpose?: string | null;
    sourceSiteFocusZoneId?: string | null;
  } | null;
}
...
export default function MapFeaturePanel({
  ...
  const ensureSelectionWorkZone = async () => {
    ...
    if (isSiteModeBound && !reusableExportZoneId) {
      ...
      const exportZones = (Array.isArray(zonesResponse?.data)
        ? zonesResponse.data
        : []) as ExportWorkZoneCandidate[];
  ...
  const handleGenerateSiteScene = async () => {
    ...
    try {
      ...
      updateNodeFromWorkZone(
        bakedFocusZone as unknown as TerrainWorkZoneDto,
        targetProject.id,
      );
