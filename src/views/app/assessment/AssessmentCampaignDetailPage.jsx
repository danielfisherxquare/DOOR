    ...
    <div style={{ display: 'grid', gap: 16 }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', gap: 16, flexWrap: 'wrap' }}>
        ...
        <div style={{ display: 'flex', gap: 12, alignItems: 'center', justifyContent: 'flex-end', flexWrap: 'wrap' }}>
          <div style={shareCardStyle}>
            <div style={{ fontSize: 12, fontWeight: 700, color: 'var(--text-secondary)' }}>考评分发链接</div>
      ...
      <div style={twoColumnStyle}>
        <div style={{ ...cardStyle, display: 'grid', gridTemplateRows: 'auto auto 1fr auto', gap: 16, minHeight: 600 }}>
          ...
          <div style={{ maxHeight: 600, overflow: 'auto', display: 'grid', gap: 8 }}>
            {candidates.map((candidate) => (
              <label key={candidate.id} style={candidateRowStyle}>
                ...
                <div style={{ flex: 1, minWidth: 0 }}>
                  ...
                  <div style={{ fontSize: 12, color: 'var(--text-secondary)', marginTop: 4, display: 'grid', gap: 2 }}>
                    ...
                    {candidate.externalEngagementType && (
                      <div style={{ color: 'var(--success)' }}>{EXTERNAL_TYPE_LABELS[candidate.externalEngagementType] || candidate.externalEngagementType}</div>
                    )}
                  </div>
            ...
            {candidates.length === 0 && <div style={{ color: 'var(--text-secondary)', padding: 20, textAlign: 'center' }}>当前机构下暂无可选团队成员。</div>}
          </div>
        ...
        <div style={{ ...cardStyle, display: 'grid', gridTemplateRows: 'auto auto 1fr', gap: 16, minHeight: 600 }}>
          ...
          {generatedCodes.length > 0 && (
            <div style={{ padding: '12px', background: 'var(--info-soft)', border: '1px solid color-mix(in srgb, var(--info) 24%, transparent)', borderRadius: 0 }}>
              <div style={{ fontSize: 13, color: 'var(--info)', marginBottom: 8, fontWeight: 700 }}>本次生成</div>
              ...
              {(selectedMemberReport.report.itemAverages || []).map((item) => {
                ...
                return (
                  <tr key={item.itemId} style={{ borderTop: '1px solid var(--border)' }}>
                    <td style={tdStyle}>
                      {item.title}
                      {isCore && <span style={{ color: 'var(--accent)', marginLeft: 4, fontSize: 11 }}>核心项</span>}
                    </td>
      ...
      <div style={cardStyle}>
        ...
        {growthReport?.timeline?.length ? (
          <table style={{ width: '100%', borderCollapse: 'collapse' }}>
            ...
            <tbody>
              {growthReport.timeline.map((row) => {
                ...
                return (
                  <tr key={`${row.campaignId}-${row.year}`} style={{ borderTop: '1px solid var(--border)' }}>
          ...
          <div style={{ color: 'var(--text-secondary)' }}>暂无成长数据</div>
        )}
      </div>
    </div>
  )
