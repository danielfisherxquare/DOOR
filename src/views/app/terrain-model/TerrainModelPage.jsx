...
function getReliefModeLabel(value) {
...
function ReliefScaleMeter({ model, recommendation, options }) {
...
function PrintReadinessPanel({ readiness }) {
...
function getBambuHandoffFromManifest(manifest) {
  if (!manifest?.print?.bambu) return null
  return manifest.print.bambu || null
}

function BambuHandoffCard({ handoff, files = [], disabled = false, onDownload }) {
...
function DeliveryPanel({ files, manifest, model, exportBlocked, onDownload }) {
  const bambuHandoff = useMemo(() => getBambuHandoffFromManifest(manifest), [manifest])
  const groups = ['推荐下载', '备用 3MF', '可打印部件', '预览文件', '记录与说明']
    .map((label) => ({
      label,
      files: files.filter((file) => getDeliveryGroup(file.name) === label),
    }))
